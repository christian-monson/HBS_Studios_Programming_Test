...
namespace Question2_CSV {
	public class SumAndMean_Tests {
		...
		[Fact]
		public void GetColumnInt_Test() {
			csvTable.Read(Path.Combine(baseDir, "test_1_normal.csv"));
			List<string> column2 = csvTable.GetColumn(1);
			Assert.Equal<string>(new List<string>() { "b", "e", "h" }, column2);
		}

		[Fact]
		public void GetColumnString_Test() {
			csvTable.Read(Path.Combine(baseDir, "test_1_normal.csv"));
			List<string> column2 = csvTable.GetColumn("column2");
			Assert.Equal<string>(new List<string>() { "b", "e", "h" }, column2);
		}

		[Fact]
		public void GetColumnBadInt_Test() {
			int invalidIndex = 100;

			csvTable.Read(Path.Combine(baseDir, "test_1_normal.csv"));
			Exception ex = Assert.Throws<IndexOutOfRangeException>(() => csvTable.GetColumn(invalidIndex));
			Assert.Equal($"The index {invalidIndex} is larger than the number of columns in this CSVTable: 3", ex.Message);
		}

		[Fact]
		public void GetColumnBadString_Test() {
			string nonExistentColumn = "non_existent_column";
			csvTable.Read(Path.Combine(baseDir, "test_1_normal.csv"));
			Exception ex = Assert.Throws<KeyNotFoundException>(() => csvTable.GetColumn(nonExistentColumn));
			Assert.Equal($"The CSVTable has no column named {nonExistentColumn}", ex.Message);
		}
	}
}
